...
- Self-review with `superpowers:code-reviewer`. **No blockers. 1 material issue + 2 nits addressed:**
    - **[material, fixed]** `ProjectForm.vue` Cancel button wrapped a `<SecondaryButton>` (button) inside Inertia's `<Link>` (anchor) — invalid HTML (`<a><button>`), confuses screen readers, ambiguous tab order. Replaced with a plain `<SecondaryButton type="button" @click="router.visit(cancelTo)">`. Same UX, valid markup.
    - **[material, deferred with TODO]** Slug auto-gen has a TOCTOU race under concurrent inserts (the `while exists` loop can be passed by two requests at once). Acceptable for phase-1 single-user dev. Added a `TODO(multi-team)` comment on the loop pointing future-us at a transactional retry pattern when we go multi-tenant.
    - **[nit, fixed]** Color/icon whitelist was duplicated in 4 places (`StoreProjectRequest`, `UpdateProjectRequest`, `ProjectController::formOptions`, `ProjectFactory`). Extracted to `App\Support\ProjectPalette::{COLORS, ICONS}` — single source of truth across the backend. Frontend mirror in `resources/js/lib/projectIcons.ts` documents the intentional cross-stack pairing.
    - **[nit, fixed]** `ProjectController::show()` used `request()->user()->can(...)` while every other action takes `Request $request`. Switched to injected `$request` for symmetry.
    - Reviewer also confirmed: `<component :is>` with `LucideIcon | null` is safe (we always `?? FolderKanban`); `useForm` defensive defaults are correct UX; `StatusBadge` doesn't double-case (uppercase is CSS-only); my `actingAs($user)->post(...)` test pattern bypasses the legacy CSRF-form path that fails on PHP 8.5; `bg-slate-950/40` on Index follows the established codebase convention. Skipped: extracting `transform()` into `App\Http\Resources\ProjectResource` (defer until a second consumer arrives) and a sidebar `overview.*` clarifying comment (already implicit).
